...
def en_passant(piece_view, colour):
    for j in piece_view:
        if j == piece:
            for current_position in piece_view[j]:
                if current_position[0] == colour[0]:
                    piece_view[piece][piece_view[piece].index(current_position)] = piece_view[piece][piece_view[piece].index(current_position)].replace(current_position, colour[2:4])
                    delete(piece_view, colour[2:4])

        if 'e.p.' in turn:
            white = white[:4]
            piece = 'P' 
            en_passant(piece_view, white, piece) 
        
        if 'e.p.' in turn:
            black = black[:4]
            piece = 'p' 
            en_passant(piece_view, black, piece)
...
moves = pre_process_moves(pgn_to_move('pgn.txt'))
piece_v = setup()
print(moves)
print(piece_v)
